...
ZERO = 0
ONE = 1
# invalid url name table
STOP = ['/wiki/N/a', '/wiki/Flag_of_the_Caribbean_Community']
...
class DFSCrawler:
    def __init__(self, url_start=START_URL, thread_num=THREADS_NUM, max_url_num=MAX_URL_NUM, max_depth=MAX_DEPTH):
        ...
        # use a stack structure to initialize frontier
        self.frontier_stack = [[self.url_start, self.maximum_layer_number]]
        self.seen_list = []
    ...
    def dict_build(self):
        file_object = open(os.getcwd() + '/wiki/dfs_refer_wiki.txt')
    ...
    # a DFS - based crawler which reaches the max depth of six.
    def dfs_wiki_crawler(self):
        current_url = self.frontier_stack.pop()
        links = self.get_link(current_url[ZERO])[::-1]
        ...
        while layer_number <= self.max_depth and len(self.seen_list) < self.max_url_num:
            ...
            for link in links:
                if 'href' in link.attrs:
                    # if seed url is from max depth,
                    # all links in it will not be pushed in the frontier stack
                    if layer_number != self.max_depth:
                        # [url, layer number of url] is the unit of the frontier
                        self.frontier_stack.append([link.attrs['href'], layer_number + 1])
            ...
            while (current_url[ZERO] in self.seen_list and self.frontier_stack) or (current_url[ZERO] in STOP):
                current_url = self.frontier_stack.pop()
    ...
    def controller(self):
        # Initialize
        url_file = open(os.getcwd()+"/wiki/dfs_wiki.txt", "w")
        in_link_file = open(os.getcwd()+"/report/dfs_in_link_graph.txt", "w")
        # call multi-threads
        start_time = datetime.datetime.now()
        t = locals()
        for i in range(self.thread_num):
            t['x%s' % i] = threading.Thread(target=self.dfs_wiki_crawler)
        ...
        with open(os.getcwd()+'/json_file/dfs_out_link_dict.json', 'w') as outfile:
            json.dump(self.out_link_dict, outfile)
        with open(os.getcwd()+'/json_file/dfs_in_link_dict.json', 'w') as outfile:
